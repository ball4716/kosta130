...
public class JoinForm extends JFrame {
	...
	public UserInfo validCheck() {
		String id = tf_id.getText().trim();
		String pass = new String(tf_pass.getPassword()).trim();
		String pass2 = new String(tf_pass2.getPassword()).trim();
		String name = tf_name.getText().trim();
		String ssn1 = tf_ssn1.getText().trim();
		String ssn2 = new String(tf_ssn2.getPassword()).trim();
		String phone;
		String addr = tf_addr.getText().trim();
		String job = cb_job.getSelectedItem().toString();

		UserInfo user = null;

		if (id == null || id.length() == 0) {// 빈값체크
			tf_id.requestFocus();
			showMsg("아이디입력!!");
		} else if (pass.length() == 0) {// 빈값체크
			tf_pass.requestFocus();
			showMsg("비번입력!!");
		} else if (pass2.equals("")) {// 빈값체크
			tf_pass2.requestFocus();
			showMsg("비번입력!!");
			...
			showMsg("이름입력");
		} else if (!(ssn1.matches("[\\d]{6}")) || !(ssn2.matches("[\\d]{7}"))) {
			tf_ssn1.setText("");
			tf_ssn2.setText("");
			tf_ssn1.requestFocus();
			showMsg("잘못된 주민번호입니다!!");
		} else if (!(tf_phone1.getText().matches("[0-9]{2,3}") && tf_phone2.getText().matches("[0-9]{3,4}")
				&& tf_phone3.getText().matches("[0-9]{4}"))) {
			tf_phone1.setText("");
			tf_phone2.setText("");
			tf_phone3.setText("");
			showMsg("잘못된 전화번호입니다!!");
			...
			phone = tf_phone1.getText().trim() + "-" + tf_phone2.getText().trim() + "-" + tf_phone3.getText().trim();
			user = new UserInfo(id, pass, name, ssn1, ssn2, phone, addr, job);
			return user;
		}
		return user;
	}

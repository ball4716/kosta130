...
<script type="text/javascript">
	...
	function pass_check(updel){
		childWin = window.open('guest/pass_check.jsp','check','width=250,height=100,top=100,bottom=150');
		state=updel;
	}
		...
		if(state=='del'){
			...
			if(confirm('정말삭제하시겠습니까?')){
				if ( (navigator.appName == 'Netscape' && navigator.userAgent.search('Trident') != -1) || (agent.indexOf("msie") != -1) ) {
					location.href="../crud.do?action=delete&id=${id }";
				} else {
					location.href="../crud.do?action=delete&id=${id }";
					alert('${id}');
				}
...
<body>
	...
	<form action="crud.do?action=update&id=${id }" method="POST" name="upForm">
	${id }
